...
function (angular, app, _, $, kbn) {
  ...
  module.controller('apipies', function($scope,$translate, $timeout, timer, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      exportfile: true,
      editorTabs : [
  ...
  module.directive('apipiesChart', function(querySrv,dashboard,filterSrv) {
    return {
      restrict: 'A',
		...
		if(scope.data.length === 0){
         ...
         if(scope.data.length === 0){
			  ...
			  if(scope.panel.chart === 'radar') {
				  ...
				  for (var i2 = 0; i2 < scope.label.length; i2++) {
				  
				  radarlabel[i2] = {name:scope.label[i2],max:scope.maxdata};
				  }
	 ...
	 if(scope.data.length === 0){
       ...
       if(scope.panel.chart === 'ebar') {
                ...
                if(scope.panel.chart === 'liquidfill') {
                    ...
                    for (var i1 = 0; i1 < scope.radardata.length; i1++) {

                        var xla = scope.label[i1].replace(" ","\n");
                        var x =  (i1+0.5) / scope.radardata.length * 100 + '%';
                        ...
                        series.push({
                            ...
                            data: [{
                                // -60 到 100 度
                                name:scope.label[i1],
                                value: scope.radardata[i1]  / scope.maxdata,
                                rawValue: scope.radardata[i1]
                            }],
                            itemStyle: {
                                normal: {
                                    shadowBlur: 0
