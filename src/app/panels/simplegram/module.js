...
function (angular, app, $, _, kbn, moment, timeSeries) {
  ...
  var module = angular.module('kibana.panels.simplegram', []);
  app.useModule(module);

  module.controller('simplegram', function($scope,$translate, $q, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
        ...
        if (dashboard.current.services.query.ids.length >= 1) {
            $q.all(mypromises).then(function (results) {
                ...
                _.each(arr_id, function (id, index) {
                    ...
                    if ($scope.panel.mode !== 'values' || $scope.panel.mode !== 'value') {
                        $scope.data[i] = {
                            info: {alias: $scope.panel.value_field, color: "#1a93f9"},
                            time_series: time_series,
                            hits: hits
                        };
                    }
